...
export const PayrollSchema = z
  .object({
    ...
    baseSalary: z.preprocess(
      (val) => (val === '' ? undefined : Number(val)),
      z.number({ required_error: 'Base salary is required.' })
        .positive('Base salary must be positive.')
    ),
    extraHours: z.preprocess(
      (val) => (val === '' ? 0 : Number(val)), // Default to 0 if empty
      z.number({ invalid_type_error: 'Extra hours must be a number.' })
        .nonnegative('Extra hours cannot be negative.').optional().default(0)
    ),
    deductions: z.preprocess(
      (val) => (val === '' ? 0 : Number(val)), // Default to 0 if empty
      z.number({ invalid_type_error: 'Deductions must be a number.' })
        .nonnegative('Deductions cannot be negative.').optional().default(0)
    ),
    totalPayment: z.preprocess(
      (val) => (val === '' ? undefined : Number(val)),
      z.number({ required_error: 'Total payment is required.' })
