...
| 7 workflows `disabled_manually` (API 2026-08-30; `opencode-review.yml` → ACTIVE 2026-09-14) | security/deploy/release/preview/scheduled/ci-enterprise existen pero NO corren (toggle GitHub apagado). Activos: `ci.yml` + `opencode-review.yml` + config dependabot. Ver §3.4/§5.9                                    | Un push a main hoy solo dispara gobernanza (+ review IA advisory en PRs); nada de security/deploy/release/preview |
...
- **Supply chain (4.3):** steps inline instalan el binario pineado **v1.18.31** vía script oficial `curl -fsSL https://opencode.ai/install | bash -s -- --version 1.18.31` (detecta OS/arch, instala a `~/.opencode/bin`; mismo trade-off mutable que el download-script de actionlint en ci.yml — el pin de BINARIO lo mantiene upstream). Antes: `anomalyco/opencode/github@5d5c35ee71c095464b9eb3c3e991df906f12a152` (SHA-pinned 2026-09-04, verificado GPG-signed). **Caveat (issue #39163, aplica al diseño anterior):** SHA-pinning protegía contra tampering de action.yml/scripts pero NO contra binary supply chain — el binario opencode se resolvía desde GitHub releases API en runtime, no desde el SHA fijado. El diseño inline elimina la action intermedia (menor superficie); si el script upstream cambia, revisar el step `Install opencode`. (Fix 2026-09-16: el intento previo de descarga directa del asset `opencode-linux-x64.zip` falló con 404 — en Linux el asset es `opencode-linux-x64.tar.gz`; el install script evita este acoplamiento al nombre del asset.)
...
| `anomalyco/opencode/github`       | **ELIMINADO 2026-09-14** (era `@5d5c35ee…`, SHA-pin) | ~~opencode-review.yml~~ → steps inline: install script oficial pineado v1.18.31 (`opencode.ai/install --version 1.18.31`) + `opencode github run` (fix rate-limit, §9.3.8) |
