...
- **Modelo (config):** `anomalyco/opencode/github` usando Groq `groq/compound-mini` (free, ~30 RPM, 70K TPM) vía **dos mecanismos complementarios**: (1) el **input `model: groq/compound-mini`** de la action (`anomalyco/opencode/github` lee ese input para setear la variable de entorno `MODEL` que opencode consume al ejecutar `opencode github run`; **sin ese input, el job falla con `Environment variable "MODEL" is not set`** — bug detectado en el primer run real 2026-09-03) y (2) el env **`OPENCODE_CONFIG_CONTENT`** que define el provider Groq. Groq **no es built-in** en opencode → requiere `@ai-sdk/openai-compatible` + baseURL (`https://api.groq.com/openai/v1`) + `apiKey: {env:GROQ_API_KEY}` en el JSON del env. Requiere el secret **`GROQ_API_KEY`**. **Nota (2026-09-02):** el modelo anterior `groq/llama-3.3-70b-versatile` fue deprecado por Groq (2026-08-16, pasó a Enterprise-only → 404 con key free/developer) y se sustituyó por `gpt-oss-120b` (modelo free-tier actual de Groq). **Fix (2026-09-03):** añadido el input `model: groq/gpt-oss-120b` a la action para resolver el error `MODEL is not set`; coherente con el cambio `ci-opencode-code-review/design.md` que ya advertía "The `model` input works correctly (unlike `agent`)" — el input quedó sin usar en la implementación inicial y era el que faltaba. **Fix 2 (2026-09-04):** al habilitar el workflow, el job avanzó más allá del MODEL pero falló con `GITHUB_TOKEN environment variable is not set. When using use_github_token, you must provide GITHUB_TOKEN.` → **con `use_github_token: true` la action exige la env var `GITHUB_TOKEN`** (el `permissions: pull-requests: write` a nivel workflow por sí solo NO inyecta la variable al step). Se añadió `GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}` al bloque `env:` del step. **Fix 3 (2026-09-04):** al llegar a la llamada real a Groq, la API respondía HTTP 404 `model_not_found` para `gpt-oss-120b`. **Causa raíz:** el ID real del modelo GPT-OSS 120B en la API de Groq es **`openai/gpt-oss-120b`** (con prefijo `openai/`), no `gpt-oss-120b`. En opencode, la clave del objeto en `models` se usa como modelID en la llamada API — el campo `modelID` separado NO es efectivo. **Fix:** cambiar la clave del modelo en `OPENCODE_CONFIG_CONTENT` de `gpt-oss-120b` a `openai/gpt-oss-120b`, y actualizar el input `model` de la action a `groq/openai/gpt-oss-120b`. **Fix 4 (2026-09-04):** `openai/gpt-oss-120b` es un modelo de razonamiento que produce `reasoning_content` en sus respuestas. Groq free-tier tiene TPM 8K (insuficiente para PRs grandes con 22 archivos) y no acepta `reasoning_content` en mensajes multi-turn → HTTP 413 (TPM exceeded) + HTTP 400 (reasoning_content unsupported). **Fix:** cambiar a `groq/compound-mini` (modelo agentic, 70K TPM, multi-turn nativo, sin reasoning_content). Para ver los modelos disponibles de tu key: `curl -s https://api.groq.com/openai/v1/models -H "Authorization: Bearer $GROQ_API_KEY" | jq -r '.data[].id'` o https://console.groq.com/docs/models.
...
| `anomalyco/opencode/github`       | **@latest**                       | opencode-review.yml (code review IA; `@latest` → SHA-pinnear tras primer run exitoso, §9.3.8/§5.4)                              |
