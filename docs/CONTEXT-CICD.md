...
- **Modelo (config):** `anomalyco/opencode/github` usando Google Gemini `google/gemini-2.5-flash` (free, 10 RPM, 250K TPM, 1M context) vía **dos mecanismos complementarios**: (1) el **input `model: google/gemini-2.5-flash`** de la action (`anomalyco/opencode/github` lee ese input para setear la variable de entorno `MODEL` que opencode consume al ejecutar `opencode github run`; **sin ese input, el job falla con `Environment variable "MODEL" is not set`** — bug detectado en el primer run real 2026-09-03) y (2) el env **`OPENCODE_CONFIG_CONTENT`** que define el provider Google. Google Gemini es **built-in** en opencode → no requiere npm custom ni configuración compleja. Requiere el secret **`GEMINI_API_KEY`** (obtenible en https://aistudio.google.com/apikey). **Nota (2026-09-02-04):** el workflow pasó por varios modelos de Groq que fallaron: `groq/llama-3.3-70b-versatile` (deprecado), `gpt-oss-120b` (404 + reasoning_content), `openai/gpt-oss-120b` (413 TPM + 400 reasoning_content), `groq/compound-mini` (404). Decisión final: migrar a Google Gemini que es built-in, tiene 250K TPM, 1M context, y no tiene issues de reasoning_content.
