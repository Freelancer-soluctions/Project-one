...
| 2026-09-21               | `ci-release-action-v2-fix`      | Repara `release.yml`: pin `changesets/action@v1.9.0` (compatible con `@changesets/cli` v2), inputs v1 (`version`/`title`/`commit`), `commitMode: github-api`, token vía input `github-token` (v2 #674 eliminó `env: GITHUB_TOKEN`) |
| 2026-09-22               | `ci-changesets-cli-v3-upgrade`  | Pareja soportada: `@changesets/cli` ^3.0.3 + `changesets/action@v2.1.2` (pin exacto), inputs v2, `push-with-git-cli: false`, `privatePackages: true` (#2186), `release` → `changeset git-tag` (#2128), engines node `>=22.11`      |
| 2026-09-21 (archivado)   | `ci-release-workflow-signing`   | Corrección de mecanismo: la config SSH en `release.yml` era dead code (API mode auto-firma web-flow); D7 corregida (App token opcional); spec delta `commit-signing-release-migration` sincronizada al spec main                   |
...
| `ci-release-action-v2-fix`                                    | ✅ Aplicado          | `release.yml` reparado (pin `@v1.9.0`); run verde pendiente de re-habilitar el workflow (task 4.1) |
...
| `anomalyco/opencode/github`       | **ELIMINADO 2026-09-14** (era `@5d5c35ee…`, SHA-pin) | ~~opencode-review.yml~~ → steps inline: install binario pineado v1.18.31 + `opencode github run` (fix rate-limit, §9.3.8)       |

**Actions de terceros:**

| Action                                            | Versión                              | Workflow / uso                                                                                                                                                                                                                        |
| ------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `changesets/action`                               | **@v2.1.2** (pin exacto, 2026-09-22) | release.yml (versionado de paquetes, modo API) con `@changesets/cli` ^3.0.3. Historial: @v2 roto 08→09/2026 → pin @v1.9.0 → pareja soportada v2↔CLI v3. Regla: action v1.x ↔ CLI v2; action v2.x ↔ CLI v3 (subir ambos en un solo PR) |
...
## Coverage Baselines (Jul 2026) — Integración ci-quality-gates

Medidos `npm run test:coverage`; alimentan thresholds `ci-test-integration`.
...
- Nota: thresholds configurados en cambio `ci-test-integration`; deben ser ≤ baselines.
