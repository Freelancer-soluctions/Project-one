...
- **Modelo (config):** `anomalyco/opencode/github` usando Google Gemini `google/gemini-2.5-flash` (free, ~15 RPM, 1M context) vía **dos mecanismos complementarios**: (1) el **input `model: google/gemini-2.5-flash`** de la action (`anomalyco/opencode/github` lee ese input para setear la variable de entorno `MODEL` que opencode consume al ejecutar `opencode github run`; **sin ese input, el job falla con `Environment variable "MODEL" is not set`** — bug detectado en el primer run real 2026-09-03) y (2) el env **`OPENCODE_CONFIG_CONTENT`** que define el provider Google. Google Gemini es **built-in** en opencode → no requiere npm custom ni configuración compleja. Requiere el secret **`GEMINI_API_KEY`** (obtenible en https://aistudio.google.com/apikey). **API key env (fix 2026-09-04):** el primer run real con Gemini falló con `AI_LoadAPIKeyError: Google Generative AI API key is missing. Pass it using the 'apiKey' parameter or the GOOGLE_GENERATIVE_AI_API_KEY environment variable` → **`@ai-sdk/google` espera `GOOGLE_GENERATIVE_AI_API_KEY`** (no `GEMINI_API_KEY`). Se configuran las **3 variantes** (`GEMINI_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY`, `GOOGLE_API_KEY`) apuntando al mismo secret para compatibilidad. **Small model (fix 2026-09-04):** el agente `title` resolvía `gemini-3.8-flash` del catálogo models.dev (modelo más barato/rápido auto-seleccionado); se fija `small_model: google/gemini-2.5-flash` en el config para determinismo. **Thinking mode** habilitado con `thinkingBudget: 8192` para mejorar la calidad del review en código complejo. **Nota (2026-09-02-04):** el workflow pasó por varios modelos de Groq que fallaron: `groq/llama-3.3-70b-versatile` (deprecado), `gpt-oss-120b` (404 + reasoning_content), `openai/gpt-oss-120b` (413 TPM + 400 reasoning_content), `groq/compound-mini` (404). Decisión final: migrar a Google Gemini que es built-in, tiene ~15 RPM free tier, 1M context, y no tiene issues de reasoning_content.
...
- **Supply chain (4.3):** se usa `anomalyco/opencode/github@5d5c35ee71c095464b9eb3c3e991df906f12a152` (SHA-pinned 2026-09-04, verificado GPG-signed). **Caveat (issue #39163):** SHA-pinning protege contra tampering de action.yml/scripts pero NO contra binary supply chain — el binario opencode se resuelve desde GitHub releases API en runtime, no desde el SHA fijado.
