...
| M6 | Node version inconsistente | `release.yml` hardcodea Node 20 vs `.nvmrc` | Unificar a `node-version-file: .nvmrc` <!-- FIXME: release.yml usa `node-version: 20` hardcoded; corregir a `node-version-file: '.nvmrc'` como los demás workflows --> |
