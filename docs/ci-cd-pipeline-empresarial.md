...
└─────────────────────────────────────────────────────────────────────────────────────────┘
 ...
 [STAGE 1: PRE-COMMIT HOOKS (local — skippable via --no-verify) [SL]]
       ...
       │          └─ GOVERNANCE: Commit lint (commitlint — Conventional Commits)
        ...
        │  │     └─ Early-abort gate (SAST diff-scoped, --baseline-commit)           │
        │  │                                                                         │
...
│                                                                                      │
│  ┌────────────────────────────────────────────────────────────────────────────────┐  │
│  │  INFRAESTRUCTURA (DAG router — habilita el resto del pipeline)                 │  │
│  │  └─ Path scoping: repo-discovery "Detect Changes" (dorny/paths-filter@v4)      │  │
│  │     · outputs booleanos client/server/e2e/shared → 20+ jobs con needs:         │  │
│  │     · alimenta DAG condicional + ci-complete · NO check governance [§4.11]     │  │
│  └────────────────────────────────────────────────────────────────────────────────┘  │
│  ┌────────────────────────────────────────────────────────────────────────────────┐  │
...
│  │  ├─ PR Metadata Checks (DCO sign-off, title/body templates)                    │  │
│  │  └─ Early-abort gate (SAST diff-scoped) — runtime ci.yml [DD][F1]:             │  │
│  │     ├─ Alcance: diff del PR (--baseline-commit, fail-on-new-only)               │  │
│  │     ├─ Severidad: ERROR (SQLi, RCE, secrets hardcoded) — naturaleza SECURITY    │  │
│  │     ├─ Tiempo: segundos. Non-blocking → blocking tras ≥1 run                    │  │
│  │     └─ Si falla → aborta pipeline ANTES de suites largas.                       │  │
│  │     (regla 4: SAST = SECURITY; regla 6: precedente dependency-review).          │  │
│  └────────────────────────────────────────────────────────────────────────────────┘  │
│  ┌────────────────────────────────────────────────────────────────────────────────┐  │
│  │  TESTING                                                                       │  │
...
│  │  ├─ SAST 3 capas [DD]: L1 staged pre-commit (CON diff) → L3 CI diff PR         │  │
│  │  │   (--baseline-commit, fail-on-new) → full semanal SIN diff (RA-5).           │  │
...
> **Decisión (2026-09-06, research):** el **early-abort gate SAST** corre como **PR gate diff-scoped en `ci.yml`**, NO en un stage "security prebuild". Razones: (1) el early-abort apunta a feedback <2 min → pertenece al nivel de PR check, no a un build stage; (2) `security.yml` está `disabled_manually` → invisible para la gobernanza (un control que no corre no es un gate); (3) "prebuild" se asocia a scans profundos/comprehensiveness, no a velocidad; (4) para que el gate tenga efecto debe ser un status check ligado al ruleset, lo que exige ≥1 run exitoso previo y nombre EXACTO del job (reglas 7 y 8 de CONTEXT-CICD).
>
> **Naturaleza vs runtime:** SAST es **SECURITY por naturaleza** (regla 4), pero su **runtime vive en el workflow de governance** (`ci.yml`) — regla 6, mismo precedente que `dependency-review` inline en `ci.yml`.
>
> **Diff-scoped vs full scan (complementarios, no duplicados):**
>
> | Dimensión        | Diff-scoped (PR gate)                                                          | Full scan (scheduled/nightly) |
> | ---------------- | ------------------------------------------------------------------------------ | ----------------------------- |
> | Costo CI         | Bajo                                                                           | Alto                          |
> | Tiempo           | <30 s (Semgrep, pocas reglas)                                                  | 3–15 min                      |
> | Cobertura        | Solo líneas cambiadas (`--baseline-commit`, fail-on-new-only, severidad ERROR) | Codebase entero               |
> | Falsos positivos | Bajos (solo hallazgos nuevos)                                                  | Altos (deuda preexistente)    |
> | Falsos negativos | Medios                                                                         | Bajos                         |
> | Cuándo           | Pre-commit / PR gate / merge-blocking                                          | Nightly / release / audit     |
>
> Principio: **"block on new risk, not the backlog"** — el PR gate bloquea riesgo nuevo; el full scan (deuda backlog) corre scheduled/nightly.
>
> **3 capas complementarias:** L1 Semgrep staged local (pre-commit, sub-segundo) → L3 CI diff-scoped en PR (<30 s) → full scan scheduled (nightly). Complementarias, no duplicadas.
>
> **Activación rolling:** `continue-on-error: true` (non-blocking) → ≥1 run exitoso → nombre EXACTO del job añadido al ruleset → blocking tras 2–4 semanas de tuning.
>
> Fuentes: [security gates en CI/CD (safeguard.sh, 2026-07-02)](https://safeguard.sh/resources/blog/security-gates-in-ci-cd), [Semgrep SAST CI pipeline gate (stackharbor.com, 2026-08-04)](https://stackharbor.com), [Semgrep CI overview](https://semgrep.dev/docs/semgrep-ci/overview), [diff-aware scans (Semgrep docs)](https://docs.semgrep.dev), [SAST complete guide (zeropath.com, 2026-06-02)](https://zeropath.com), [DevSecOps shift-left (mdsanwarhossain.me, 2026-08-12)](https://mdsanwarhossain.me), [GitHub rulesets — available rules](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets).
...
| **SECURITY**        | SAST (Semgrep, CodeQL)                                    | **F1 non-blocking** — job `SAST (Semgrep)` en `ci.yml` con `continue-on-error: true`, `--severity ERROR --fail-on error`, `--baseline-commit diff-scoped` y 9 packs inline + `--config .semgrep/rules` reglas custom (CWE-1321/918/502/89/79). Capa governance standalone, NO gated por `CI_MINIMAL`, fuera de `ci-complete.needs`. Custom rules en `.semgrep/rules/` cargadas antes que los packs remotos. | **ANTES del build**      | [Project-one ci-sast-governance](openspec/changes/ci-sast-governance/)                                                                                                                                                                                                                                                          |
...
| **GOVERNANCE**      | Path-filtered entry / Skip-CI (workflow triggers)         | **NO** — change detection en source                                                                                                                                                                                                                                                                                                                                                                         | **ENTRY**                | [§53.3 Skip unnecessary builds](https://docs.aws.amazon.com/wellarchitected/latest/devops-guidance/)                                                                                                                                                                                                                            |
| **INFRAESTRUCTURA** | Path scoping — job repo-discovery (dorny/paths-filter@v4) | **NO** — outputs del diff del PR                                                                                                                                                                                                                                                                                                                                                                            | **PRE-BUILD (PR GATE)**  | Implementación real: `repo-discovery` (Detect Changes) en ci.yml, primer job del PR gate; 20+ jobs con `needs: repo-discovery`; alimenta DAG condicional y `ci-complete` (pre-merge-gates-governance.md §4.11)                                                                                                                  |
...
| **SECURITY**        | Early-abort gate (SAST diff-scoped) [DD]                  | **NO** — aborta antes de suites largas                                                                                                                                                                                                                                                                                                                                                                      | **PR GATE (ci.yml)**     | Naturaleza SECURITY (regla 4) aunque runtime en workflow governance (regla 6 — precedente dependency-review inline en ci.yml). Diff-scoped `--baseline-commit`, fail-on-new-only, severidad ERROR; segundos. NO corre en security.yml (`disabled_manually`). Non-blocking → blocking tras ≥1 run exitoso + tuning (reglas 7/8). |
...
| **INFRAESTRUCTURA** | repo-discovery (Detect Changes)  | Pre-Build          | CI           | Primer job del PR gate — path scoping dorny/paths-filter v4 |
...
- Implementación real en project-one: el job `repo-discovery` (name: "Detect Changes") en `ci.yml` replica este patrón con filters `client`/`server`/`e2e`/`shared` (`apps/client/**`, `apps/server/**`, `e2e/**`, `package.json` + lockfile + `.github/workflows/**`). 20+ jobs usan `needs: repo-discovery`; `ci-complete` lo incluye en su agregación.
...
| **Early-abort gate (SAST diff-scoped)** | RA-5 (Vulnerability Scanning)   | Vulnerability management — fail-fast para amenazas críticas | Semgrep results (ERROR only, diff-scoped)      |
