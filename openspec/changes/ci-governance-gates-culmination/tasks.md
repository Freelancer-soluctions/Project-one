...
- [ ] 1.1 Create `.github/workflows/dependency-review.yml` running `actions/dependency-review-action` on `pull_request`, failing on new moderate+ vulnerabilities. Add job name `dependency-review` to `ci-complete.needs` array. **Job name MUST match the ruleset required-check name exactly** (per ci-pr-metadata-governance D4 job-name-matching risk). **Esfuerzo: 2-3h**
- [ ] 1.2 Verify PR Title Lint exists with `continue-on-error: true` — will become blocking in Phase 2 of rollout (per ci-pr-metadata-governance D7). **Esfuerzo: 0h (verify only)**
...
| 2.2  |  L3   |    Document     | Admin-1         | Consistent with ci-pr-metadata-governance D3         |
...
**Blocked by admin**: 2.1, 2.2 (2 tasks — COMBINED with ci-pr-metadata-governance)
