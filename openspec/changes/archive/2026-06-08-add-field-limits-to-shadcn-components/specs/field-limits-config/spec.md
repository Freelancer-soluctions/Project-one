...
### ProviderOrder Module
- notes: maxLength 200

## Filter Forms

Filter forms (search bars, filter panels) also use FIELD_LIMITS:

- `search.searchTerm: 100` — used in NotesFilters and EventFiltersForm to limit search input length
- All filter forms are migrated to use FIELD_LIMITS from the centralized config

The following 10 filter forms use FIELD_LIMITS:
- UsersFiltersForm
