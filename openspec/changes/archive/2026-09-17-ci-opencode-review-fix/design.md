...
- **Alternatives considered:**
  ...
  2. Use `latest` tag — reintroduces the same dynamic‑version problem we are fixing.

**Decision 3: Installation location**

- **Chosen:** Install to `$HOME/.opencode/bin` and add to `GITHUB_PATH`.
- **Rationale:** Follows the conventional location used by the opencode installer; ensures the binary is in `PATH` for subsequent steps. Avoids writing to system directories.
- **Alternatives considered:**
...
**Risk: Opencode release URL may change or become unavailable**
→ Mitigation: Pin to a specific version (v1.18.31); GitHub releases are immutable. If the URL changes, update the install step. Monitor the opencode repository for deprecation notices.

**Risk: Binary compatibility – the pinned version may not support all features**
→ Mitigation: v1.18.31 is the latest stable release (verified). If a newer version is required, update the URL and test locally before merging.
