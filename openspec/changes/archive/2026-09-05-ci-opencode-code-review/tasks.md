...
- [x] 1.3 Add `anomalyco/opencode/github@5d5c35ee71c095464b9eb3c3e991df906f12a152` step (SHA-pinned for supply chain safety, verified 2026-09-04) with `use_github_token: true`, `model: google/gemini-3.6-flash` (sets `MODEL` env var), `OPENCODE_CONFIG_CONTENT` env var (Google Gemini built-in provider — see concrete JSON in task 2.2), and `GEMINI_API_KEY` secret (mapped to 3 env vars)
...
- [x] 1.10 Smoke test with a PR: verify the comment appears/updates and does NOT block merge — **PASS 2026-09-05**

## 2. Secret and Configuration

- [x] 2.1 Add `GEMINI_API_KEY` to GitHub repo secrets (free at aistudio.google.com/apikey; mapped to 3 env vars: `GEMINI_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY`, `GOOGLE_API_KEY`) — confirmado por el usuario (2026-09-05)
- [x] 2.2 Configure `OPENCODE_CONFIG_CONTENT` env var with the following Google Gemini configuration (Google Gemini is a BUILT-IN provider — no npm custom package, no baseURL, no apiKey needed):
...
> **Historial de migración (consolidado):** (1) 2026-09-02: Groq `llama-3.3-70b-versatile` deprecado (Enterprise-only → 404 free/developer); sustituido por `gpt-oss-120b`. (2) 2026-09-03: `model: groq/gpt-oss-120b` necesario para setear `MODEL` env var (fix `Environment variable "MODEL" is not set`). (3) 2026-09-04: migrado a Google Gemini `gemini-2.5-flash` (built-in); fix env `GOOGLE_GENERATIVE_AI_API_KEY`; fix `small_model`. (4) 2026-09-05: `gemini-2.5-flash` 404 `"no longer available to new users"` → migrado a `gemini-3.6-flash` (GA julio 2026); removido `thinkingConfig` (400 thinking budget/level conflict). La action requiere input `model: google/gemini-3.6-flash` para setear `MODEL` env var. 3 env vars mapeadas al mismo secret.

- [x] 2.3 Document fallback model (`opencode/free`) and rate-limit exhaustion behavior (Google Gemini `gemini-3.6-flash` primary or fallback to `opencode/free`; if both exhausted, post rate-limit advisory and exit gracefully) — documentado en `docs/CONTEXT-CICD.md` §9.3.8 (Rate-limit / fallback)
...
- [x] 3.2 Enable workflow after provisioning `GEMINI_API_KEY` (`gh workflow enable .github/workflows/opencode-review.yml` or UI → Actions → Enable) — confirmado por el usuario (2026-09-05)
- [x] 3.3 Verify the workflow triggers on a test PR and posts a structured comment — confirmado por el usuario (2026-09-05); smoke test final PASS en 1.10
...
- [x] 4.3 Document the pinned action version (`anomalyco/opencode/github@5d5c35ee71c095464b9eb3c3e991df906f12a152`, SHA-pinned after first successful run) and supply chain considerations (§5.4) — documentado en `docs/CONTEXT-CICD.md` §9.3.8 (Supply chain)
