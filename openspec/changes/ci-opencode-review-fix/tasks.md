...
- [ ] 1.3 Add new step "Install opencode" that runs `curl -fsSL https://opencode.ai/install | bash -s -- --version 1.18.31` (official install script, binary pinned via `--version` flag) followed by `opencode --version` as fail-fast verification. Script trade-off: upstream-mutable (same pattern as actionlint download-script in ci.yml), mitigated by version pin.
