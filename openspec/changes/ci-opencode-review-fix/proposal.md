...
- Replace the `anomalyco/opencode/github` composite action with two inline steps:
  1. **Install opencode** – download a pinned release (`v1.18.31`) from `https://github.com/anomalyco/opencode/releases`, unzip to `$HOME/.opencode/bin`, add to `GITHUB_PATH`.
...
- **SHA‑pinning**: The opencode binary version is pinned to `v1.18.31`; the install URL is hardcoded (no dynamic version lookup).
- **Supply chain**: Eliminates the dependency on the third‑party composite action (which itself fetches a binary at runtime). The new steps download a specific release from the official opencode repository.
