...
### Requirement: Production bootstrap env vars via secrets

The production task definition SHALL inject the application's bootstrap env vars through the production environment's secrets, so the encryption middleware loads without aborting the Node process before the HTTP server listens.

#### Scenario: Production task definition injects AES_GCM_KEY via Secrets Manager

- **WHEN** the production job registers the ECS task definition
- **THEN** the container definition includes `ALGORITHM=aes-256-gcm` as a non-secret `environment` entry (public constant) and `AES_GCM_KEY` as a `secrets` entry referencing `PROD_AES_GCM_KEY_SECRET_ARN` (AWS Secrets Manager ARN held by the production environment)
- **AND** the production environment exposes `PROD_AES_GCM_KEY_SECRET_ARN` alongside the existing `PROD_DATABASE_URL_SECRET_ARN`, `PROD_JWT_SECRET_SECRET_ARN`, and `PROD_AWS_REGION_SECRET_ARN`
- **AND** the deployed task passes its health check (the container must boot completely for `/health` to respond HTTP 200)
