...
- [x] 1.3 Service containers del job: `floci` (`floci/floci:1.5.31` pinneado, healthcheck `["CMD-SHELL", "curl -f http://localhost:4566/_floci/health"]`, puerto 4566, `FLOCI_HOSTNAME=floci`) + `db` (postgres:16-alpine, healthcheck `pg_isready`, credenciales `test`/`test`/`project_one_cd`) — patrón del workflow preview del sibling
