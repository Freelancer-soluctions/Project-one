...
- The workflow uses `anomalyco/opencode/github` (using `@latest` per official docs — no `v1.0.0` tag exists; SHA-pinned after first successful run for supply chain safety) with `use_github_token: true` and scoped permissions (`contents: read`, `pull-requests: write`, `issues: read`). The Groq `llama-3.3-70b-versatile` model (free tier, 30 RPM) is configured self-contained via `OPENCODE_CONFIG_CONTENT` env var with custom provider JSON (Groq requires `@ai-sdk/openai-compatible` package — it is NOT a built-in provider), with `opencode/free` as fallback.
