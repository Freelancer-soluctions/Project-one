...
- El change sibling `ci-preview-environments` ya adopta Floci (`floci/floci:v1.5.11`, puerto 4566, `FLOCI_STORAGE_MODE=memory`, healthcheck `["CMD", "floci", "health"]`) en un stack separado (`docker-compose.preview.yml`) y es dueño de `docs/aws-learning-with-floci.md`.
...
2. **Imagen pineada `floci/floci:v1.5.11` + `FLOCI_STORAGE_MODE=memory`**
...
3. **Healthcheck `["CMD", "floci", "health"]`**
...
- [Imagen pineada v1.5.11 queda desactualizada] → Mitigation: política de pinning del repo; bump controlado y versionado como cualquier dependencia
...
4. Validación manual: `docker compose up -d floci` + `docker compose ps` (estado healthy del healthcheck `floci health`) + script/REPL independiente que importa `loadSecrets()` (ver task 2.2 — `loadSecrets()` es código muerto hoy, no lo llama ningún archivo de la app; se valida vía REPL hasta que exista un consumidor)
