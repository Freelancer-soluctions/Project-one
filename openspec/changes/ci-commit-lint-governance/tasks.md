...
- [ ] 3.5 Register commit-lint as a required status check in the GitHub branch-protection ruleset (manual step) AND document the procedure
  ...
  - **Why necessary (research verdict)**: non-required checks are advisory only (merge-over-red allowed); merge queue ignores non-required check failures; direct pushes skip PR-only jobs. Residual gap: squash-merge final message not linted (optional mitigation: push-ruleset commit-message regex — requires Team/Enterprise plan for private repos). — LEAVE UNCHECKED: manual GitHub ruleset registration
