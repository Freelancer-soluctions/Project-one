...
- [x] 2.4 Poner el step del scan en modo auditoría (`continue-on-error: true`): el run no falla por hallazgos, los reporta y continúa **Nota de corrección (2026-09-22, review audit)**: la marca `[x]` original era incorrecta — el workflow entró a `main` SIN `continue-on-error` (fail-closed, violando el escenario "Secret found in historical commit" del spec). Aplicado ahora en `scheduled-security.yml` a AMBOS steps de scan (JSON + SARIF) con `continue-on-error: true` + `env: GITHUB_TOKEN` (gitleaks consulta `api.github.com` al detectar Actions y falla con 403 sin token; además garantiza que el SARIF se genere/suba aunque haya hallazgos). Validado con actionlint; confirmación en run real pendiente (task 5.2).
