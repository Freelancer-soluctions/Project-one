# local-pr-title-check Specification
...
The `pr-title-check.js` script SHALL validate that a PR title starts with one of the allowed Conventional Commits type prefixes: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`, `ops`.
...
- **WHEN** script is invoked with `node scripts/hooks/pr-title-check.js "feat: add feature"`
- **THEN** the script SHALL validate the provided title string

#### Scenario: Title piped via stdin

- **WHEN** script is invoked with `echo "feat: add feature" | node scripts/hooks/pr-title-check.js`
