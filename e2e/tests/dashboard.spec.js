...
test.describe('Dashboard Access', () => {
  ...
  test('WHEN authenticated user accesses dashboard URL THEN dashboard renders with user info', async ({
    page,
  }) => {
    await expect(page).toHaveURL(/.*home/);
  ...
  test('WHEN user clicks dashboard modules THEN navigates to correct page', async ({
    page,
  }) => {
    const modules = [
  ...
  test('WHEN dashboard loads THEN key metrics displayed', async ({ page }) => {
    const metrics = page
      .locator(
        '[data-testid="metric"], .metric, .stat, .card:has-text("Total")'
      )
      .first();
  ...
  test('WHEN user switches language THEN dashboard updates', async ({
    page,
  }) => {
    const langSwitcher = page
      .locator(
        'button:has-text("EN"), button:has-text("ES"), [role="combobox"]:has-text("Language"), [role="combobox"]:has-text("Idioma")'
      )
      .first();
    if (await langSwitcher.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      const option = page
        .locator(
          '[role="option"]:has-text("Spanish"), [role="option"]:has-text("Español")'
        )
        .first();
...
test.describe('Dashboard Responsive', () => {
  ...
  test('WHEN viewport is mobile THEN sidebar collapses', async ({ page }) => {
    ...
    const toggle = page
      .locator(
        'button[aria-label="Toggle sidebar"], button:has-text("Menu"), button:has-text("menu")'
      )
      .first();
