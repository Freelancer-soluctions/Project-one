    ...
    try {
      ...
      if (!user || !user.roles?.code) {
        return res.status(403).json({
          error: 'No se pudo verificar el rol o permisos del usuario',
        });
      }
      ...
      if (allowedRoles.length > 0 && !allowedRoles.includes(userRoleCode)) {
        return res
          .status(403)
          .json({ error: 'Rol no autorizado para esta acción' });
      }
      ...
      if (!hasSomePermission) {
        return res
          .status(403)
          .json({ error: 'Permisos insuficientes para esta operación' });
      }
      ...
      return res
        .status(500)
        .json({ error: 'Error interno en la verificación de permisos' });
    }
  };
