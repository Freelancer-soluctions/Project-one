...
bus.on(BUS_EVENTS.MENTION_CREATED, (payload) => {
  console.log(`📢 Bus → WS: entregando mención a usuario ${payload.mentionedUserId}`)
...
const simulateMention = (payload) => {
  console.log(`📝 Servicio: mención creada por usuario ${payload.mentionedByUserId} para usuario ${payload.mentionedUserId}`)
...
httpServer.listen(PORT, () => {
  ...
  console.log('   Conecta un cliente como usuario 42 para recibir la mención simulada')
  console.log('   Cliente: io("http://localhost:3005").emit("auth:simulate", { userId: 42 })')
})
