...
io.on('connection', (socket) => {
  ...
  console.log(`🟢 Socket conectado: ${socket.id} → Usuario simulado: ${userId}`);
  ...
  socket.on('disconnect', (reason) => {
    console.log(`🔴 Socket desconectado: ${socket.id} (usuario ${userId}), razón: ${reason}`);
...
httpServer.listen(PORT, () => {
  console.log(`🚀 Servidor de demostración de salas escuchando en http://localhost:${PORT}`);
  console.log(`   IDs de usuario simulados: ${SIMULATED_USER_IDS.join(', ')}`);
  console.log(`   Prueba uniendo múltiples clientes y enviando mensajes entre ellos`);
});
...
process.on('SIGTERM', () => {
  console.log('🛑 Recibida señal SIGTERM. Cerrando servidor de demostración...');
  io.close(() => {
    console.log('🔌 Servidor Socket.IO cerrado.');
    httpServer.close(() => {
      console.log('🛑 Servidor HTTP cerrado.');
