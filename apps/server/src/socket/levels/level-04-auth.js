...
io.on('connection', (socket) => {
  console.log(`🟢 Usuario autenticado conectado: ${socket.data.user?.id || 'desconocido'}`)

  socket.on('disconnect', (reason) => {
    console.log(`🔴 Usuario desconectado: ${socket.data.user?.id}, razón: ${reason}`)
  })
})
...
httpServer.listen(PORT, () => {
  ...
  console.log(`   Conectar con: io('http://localhost:${PORT}', { auth: { token: '...' } })`)
